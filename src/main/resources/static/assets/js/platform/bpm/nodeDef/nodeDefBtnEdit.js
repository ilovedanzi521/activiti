var app = angular.module("btnApp", [ 'base', 'baseDirective' ]);
		app.controller('btnController', [ '$scope', 'baseService', 'ArrayToolService', function($scope, baseService, ArrayTool) {
			...
			$scope.getInitButtons = function(){
				var get = baseService.get(__ctx + "/service/bpm/processDef/getDefaultNodeBtns?isDefault=false&defId="+$.getParam("defId")+"&nodeId="+$.getParam("nodeId"))
			...
			$scope.initBtns = function(){
				var get = baseService.get(__ctx + "/service/bpm/processDef/getDefaultNodeBtns?isDefault=true&defId="+$.getParam("defId")+"&nodeId="+$.getParam("nodeId"))
				get.then(function(data){
