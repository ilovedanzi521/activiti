...
app.controller('ctrl', [ '$scope', 'baseService', 'ArrayToolService', '$filter', function($scope, baseService, ArrayToolService, $filter) {
	...
	$scope.init = function() {
		...
		// });
		$scope.searchTableName();
		ToolsController.getEnum("com.yhfin.ycmp.bpm.api.constant.BusColumnCtrlType").then(function(data) {
			$scope.FieldControlType = data;
		});
		ToolsController.getEnum("com.yhfin.ycmp.bpm.api.constant.ColumnType").then(function(data) {
	...
	$scope.searchTableName = function() {
		var url = __ctx + "/bus/businessTable/all";
