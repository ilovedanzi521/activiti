...
export default class FundValueController extends BaseController {
    ...
    // 缓存字典项
    private fundValueDicData: any = ParamPriceInfoDic;
    /** 列表信息 */
    ...
    private cacheDict() {
         ...
         * 交易市场
         */
        dic.parentDicCodeList = ["1000102"];
        this.dicService
            .dicMultipleAllSubList(dic)
            .then((res: WinResponseData) => {
                this.fundValueDicData.marketType = res.data[1000102];
            });
    }
    ...
    private formatDic({ row, rowIndex, column, columnIndex }) {
        ...
        if (column.property === "marketCode") {
            return this.getLabelFromDic(
                row.marketCode,
                this.fundValueDicData.marketType
            );
        }
    }
    ...
    private delBatch() {
        ...
        if (multipleSelection.length > 1) {
            this.win_message_box_warning(
                ...
                const ids = [];
                multipleSelection.forEach((element: ParamFundValueRepVO) => {
                    ids.push(element.id);
                });
