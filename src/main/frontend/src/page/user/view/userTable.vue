<template>
    <win_table style="width: 100%" stripe :data="userReqVo.userArray" :showSelection="false">
        ...
        <win_table_column prop="operation" label="操作" width="480">
            <template slot-scope="scope">
                <!-- <span class="operation" @click="handleAuth(scope.row)">
                    <i class="icon icon1 el-icon-user-solid"></i>权限管理
                </span> -->
                <span class="operation" @click="handleCancel(scope.row)" v-if="scope.row.status===3"></span>
                ...
                <span class="operation" @click="handleCancel(scope.row)" v-if="scope.row.status===3"></span>
                <span class="operation" @click="handleCancel(scope.row)" v-else> <i class="icon icon4 el-icon-timer"></i>注销</span>
