...
import { ParamRateChangeEntity } from "../vo/ParamBondInfoEntity";
...
export default class InterestRateChangeInfoController extends BaseController {
    ...
    // 利率变动数据字典
    private paramRateChangeEntity = ParamRateChangeEntity;
    // 数据字典服务
    ...
    private mounted() {
        ...
        // 交易市场
        this.publicDicFun("marketType", BondConstant.MARKET_TYPE_DICCODE);
    }

    /**
     * 数据字典初始化
     */
    private publicDicFun(dicName: string, dicCode: string) {
        const dic = new DicReqVO();
        dic.parentDicCode = dicCode;
        this.dicService.dicAllSubList(dic).then((res: WinResponseData) => {
            ParamRateChangeEntity[dicName] = res.data;
        });
    }

    // 交易市场
    private marketTypeFormatter({ cellValue }) {
        return this.dicFormatter(cellValue, ParamRateChangeEntity.marketType);
    }
