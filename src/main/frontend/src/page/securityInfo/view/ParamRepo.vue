  ...
  <div style="margin-left:10px;">
      ...
      <win_form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <win_form_item label="交易市场">
          <win_select v-model="formInline.marketCode" filterable multiple placeholder="请选择" clearable>
            <win_option v-for="item in marketType" :key="item.dicCode" :label="item.dicExplain" :value="item.dicCode">
    ...
    <div>
      <win_table :data="repoInfo.list" max-height="442px" v-loading="loading" highlight-current-row element-loading-text="拼命加载中..." @select-change="tableSelectionChange" @select-all="tableSelectionChange" @cell-dblclick="dblclick">
        <win_table_column prop="securityCode" label="证券代码" width="200"></win_table_column>
        <win_table_column prop="securityName" label="证券名称" width="200"></win_table_column>
        <win_table_column prop="marketCode" label="交易市场" :formatter="formatDic" width="200"></win_table_column>
