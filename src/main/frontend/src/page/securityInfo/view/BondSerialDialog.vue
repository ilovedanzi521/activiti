<template>
    <div class="BondSerialDialog">
        ...
        <win-fdialog :title="myTitle" :visible.sync="dialogFormVisible" :close-on-press-escape="false" :close-on-click-modal="false" width="42%" :before-close="handleClose">
            <!--债券分期还本详情表单:Begin -->
            <win-form :model="form" ref="ruleForm" :rules="serialDetailRules" :disabled="allDisabled" label-width="110px" :inline="true">
                <div class="form_content">
                    ...
                    <win-form-item label="偿还日期" prop="repayDate">
                        <el-date-picker v-model="form.repayDate" value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                    </win-form-item>
                </div>
                <div class="form_content">
                    <win-form-item label="分期偿还类型" prop="amortizeType">
                        <win-select v-model="form.amortizeType" placeholder="请选择">
                            <win-option v-for="item in bondSerialDicEntity.amortizeType" :key="item.dicCode" :label="item.dicExplain" :value="item.dicCode"></win-option>
                        </win-select>
                    </win-form-item>
                    <win-form-item label="偿还比例" prop="repayRate">
                        <win-input v-model="repayRate" :num="[5,3]" :ratio="0.01" :real.sync="form.repayRate" :negative="false" unit="%" style="width:224px;"></win-input>
                    </win-form-item>
                </div>
            </win-form>
            <!--债券分期还本详情表单:End -->
