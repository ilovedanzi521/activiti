...
export default class LoginController extends Vue {
    ...
    async login(params) {
        if (this.formIsVerification(params.loginName, params.password)) {
            ...
            if (res.code == "200") {
                ...
                localStorage.setItem("Authorization", res.data); //将tonk保存到本地
    ...
    yzmTip(code: string): void {
        ...
        if (
            this.loginVO.code.toUpperCase() === this.loginVO.yzm.toUpperCase()
        ) {
            this.loginVO.rememberOk = true;
            this.loginVO.tipMessage = "";
        } else {
            this.loginVO.rememberOk = false;
    ...
    formIsVerification(userName: string, password: string): boolean {
        if (!Tool.validate(userName, "require")) {
            ...
            return false;
        } else if (!this.loginVO.rememberOk) {
