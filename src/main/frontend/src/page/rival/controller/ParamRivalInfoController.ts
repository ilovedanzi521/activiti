 ...
 * 文件名称: ParamRivalInfoController.ts
...
import ParamRivalInfoService from "../service/ParamRivalInfoService";
import PageVO from "../../common/vo/PageVO";
import {
    ParamRivalInfoReqVO,
    ParamRivalInfoRepVO
} from "../vo/ParamRivalInfoVO";
import { WinResponseData } from "../../common/vo/BaseVO";
import RivalInfoDicDataVO from "../vo/RivalInfoDicDataVO";
...
import ParamRivalInfoDialog from "../view/ParamRivalInfoDialog.vue";
import { BaseConst } from "../../common/const/BaseConst";
import { mapState } from "vuex";
/**
 * 类名称：ParamRivalInfoController
 * 类描述：交易对手表 前端控制器
 * 创建人：@author zoujian
 * 创建时间：2019-07-10 11:35:35
 */
@Component({
    components: { ParamRivalInfoDialog },
    computed: {
        ...mapState({
        ...
        showMore: {
            handler(newValue, oldValue) {
                ...
                if (newValue.flag) {
                    this.tableHeight =
                        newValue.offsetTop -
                        this.$refs.rivalInfoTable.$el.offsetTop -
                        70;
                }
            }
        }
...
export default class ParamRivalInfoController extends BaseController {
    /** service */
    private service: ParamRivalInfoService = new ParamRivalInfoService();
    /** 数据字典service */
    private dicService: DicService = new DicService();
    /** 选中信息 */
    private multipleSelection: any = [];
    /** 请求VO */
    private reqVO: ParamRivalInfoReqVO = new ParamRivalInfoReqVO();
    /** 数据字典 */
    private rivalInfoDicData: RivalInfoDicDataVO = new RivalInfoDicDataVO();
    /** dialog显示控制 */
    ...
    private cardNumber: {
        ...
        data: ParamRivalInfoRepVO;
        rivalInfoDicData: RivalInfoDicDataVO;
    };
    ...
    private query(): void {
        this.service.pageList(this.reqVO).then((res: WinResponseData) => {
            ...
            this.$nextTick(() => {
                if (
                    this.$refs.rivalInfoTable !== undefined &&
                    this.$refs.rivalInfoTable.data.length > 0
                ) {
                    // 第一行选中
                    this.$refs.rivalInfoTable.setCurrentRow(
                        this.$refs.rivalInfoTable.data[0]
                    );
                    // 同步
     ...
     */
    private reset(): void {
        this.reqVO = new ParamRivalInfoReqVO();
    }
    ...
    private formatDic({ row, column }) {
        if (column.property === "appraise") {
            return this.getLabelFromDic(
                row.appraise,
                this.rivalInfoDicData.appraiseDic
            );
        }
        if (column.property === "golden") {
            return this.getLabelFromDic(
                row.golden,
                this.rivalInfoDicData.goldenDic
            );
        }
        if (column.property === "firstSettleType") {
            return this.getLabelFromDic(
                row.firstSettleType,
                this.rivalInfoDicData.settleTypeDic
            );
        }
        if (column.property === "secondSettleType") {
            return this.getLabelFromDic(
                row.secondSettleType,
                this.rivalInfoDicData.settleTypeDic
            );
        }
    ...
    private rivalNoSelect(queryString: string, cb: any) {
        const rivalInfoReqVO = new ParamRivalInfoReqVO();
    ...
    private filter(
        ...
        const array = [];
        if (list.length > 0) {
            list.forEach((element: ParamRivalInfoRepVO) => {
     ...
     * @param type
     */
    private operation(row: ParamRivalInfoRepVO, type: string) {
        this.dialogVisible = true;
        if (type === OperationTypeEnum.ADD) {
            row = new ParamRivalInfoRepVO();
            this.cardNumber = {
