<template>
  <div>
      ...
      <win_form :model="form" :inline="true" ref="form">
        ...
        <win_form_item label="证券类别">
          <win_select v-model="form.securityType" filterable clearable placeholder="请选择">
            <win_option v-for="item in securityTypeRepVOs" :key="item.securityType" :label="item.securityTypeName" :value="item.securityType"></win_option>
          </win_select>
        </win_form_item>
        <win_form_item>
    ...
    <div style="margin-top:20px;">
      <win_table ref="multipleTable" highlight-current-row :data="pageVO.list" tooltip-effect="dark" @cell-dblclick="dblclick" @select-change="tableSelectionChange" @select-all="tableSelectionChange" indexFixed selectionFixed
        max-height="440">

        <win_table_column prop="securityCode" label="证券内码" width="100" fixed="left" sortable></win_table_column>
        ...
        <win_table_column prop="yesterdayClearing" label="昨结算" width="80"></win_table_column>
        <win_table_column prop="todayClearing" label="今结算" width="80"></win_table_column>
